...
namespace sgf
{
    ...
    public:
        ...
        bool hasComponentType(const std::type_index &type);
        ...
        template<class T, class ... Args>
        void setComponent(const std::string &ID, Args&& ... args);
        
        
        
        void inactiveComponent(const std::string &ID);
        
        void reactiveComponent(const std::string &ID);
    ...
    template <class ComponentType>
    bool sgf::Entity::hasComponent(const std::string &ID)
    {
        std::type_index index (typeid(ComponentType));
    ...
    template<class T, class ... Args>
    void sgf::Entity::addComponent(const std::string &ID, Args&& ... args)
    {
    ...
    template <class T>
    T & sgf::Entity::getComponent(const std::string &ID) const
    {
        ...
        else throw sgf::Exception("Cannot retrieve the component type asked for");
    }
    
    ///// SET //////
    
    template<class T, class ... Args>
    void sgf::Entity::setComponent(const std::string &ID, Args&& ... args)
    {
        if (!hasComponent<T>(ID))
            throw sgf::Exception("Cannot set a non existing component");
