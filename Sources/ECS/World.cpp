...
void sgf::World::runSystems(float elapsed)
{
...
{
    //check against entity's validity
    ...
    //add it to the concerned systems
    for (auto it = _systems.begin(); it != _systems.end(); ++it)
    {
...
void sgf::World::removeEntity(const indexType &ID)
{
...
void sgf::World::reactiveEntity(const indexType &ID)
{
...
void sgf::World::unactiveEntity(const indexType &ID)
{
    auto pos = _activeEntities.find(ID);
    if (pos == _activeEntities.end())
        throw sgf::Exception("Trying to access a non existing Entity");
...
bool sgf::World::isActivated(const indexType &ID) const
{
    auto pos =_activeEntities.find(ID);
    if(pos != _activeEntities.end())
    {
